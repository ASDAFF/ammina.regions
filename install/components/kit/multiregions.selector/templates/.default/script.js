(function ($) {
	...
	var methods = {
		init: function (arResultInfo, arParamsInfo) {
			...
			$("body").append($(".bam-multiregions-confirm"));
		...
		_showConfirmCityWindow: function () {
			...
			var offset = $(methods.self).find(".bam-multiregions-link").offset();
			methods.divConfirm.css("top", offset.top + $(methods.self).find(".bam-multiregions-link").outerHeight());
			methods.divConfirm.css("left", offset.left + ($(methods.self).find(".bam-multiregions-link").outerWidth() - confirmWidth) / 2);
