...
class KitMultiRegionsGeoContentComponent extends CBitrixComponent
{
	...
	public function onPrepareComponentParams($params)
	{
		...
		if ($params['PREVENT_DOMAIN_ID'] <= 0) {
			$params['PREVENT_DOMAIN_ID'] = $GLOBALS['KIT_MULTIREGIONS']['SYS_CURRENT_DOMAIN_ID'];
		}
	...
	{
		return "/kit/multiregions/geocontent/";
	}
	...
	protected function processResultData()
	{
		...
		if ($this->arParams['PREVENT_CITY'] > 0) {
			...
			$rContent = \Kit\MultiRegions\ContentTable::getList(array(
				"filter" => array(
			...
			$arData = \Kit\MultiRegions\ContentTable::getList(array(
				"filter" => array(
					"TYPE_ID" => $this->arParams['CONTENT_TYPE'],
					"ACTIVE" => "Y",
					"CITY_ID" => array(0, false),
		...
		if (!empty($arData)) {
			...
			$arData['ACTIVE_CONTENT'] = CKitMultiRegions::getLangFirstName(array_merge(array("ru" => $arData['ACTIVE_CONTENT']), $arData['ACTIVE_CONTENT_LANG']));
		}
