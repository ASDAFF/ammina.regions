...
class AmminaRegionsSelectorComponent extends CBitrixComponent
{
	...
	protected function obtainCityFormDataNoGpsQuery($query)
	{
		...
		$rCity = \Ammina\Regions\CityTable::getList(array(
			...
			"select" => $arSelect,
			"runtime" => array(
				new \Bitrix\Main\Entity\ExpressionField('POSINSTR', 'IF(LOCATE(\'' . amreg_strtolower($query) . '\', LOWER(`ammina_regions_city`.`NAME`))>0,LOCATE(\'' . strtolower($query) . '\', LOWER(`ammina_regions_city`.`NAME`)),IF(LOCATE(\'' . strtolower($query) . '\', LOWER(`ammina_regions_city_city_lang`.`NAME`))>0,LOCATE(\'' . strtolower($query) . '\', LOWER(`ammina_regions_city_city_lang`.`NAME`)),1000000000))'),
