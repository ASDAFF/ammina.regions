...
class AMREG_Mobile_Detect
{
	...
	public function getHttpHeader($header)
	{
		...
		if (amreg_strpos($header, '_') === false) {
			$header = str_replace('-', '_', $header);
			$header = amreg_strtoupper($header);
		}
	...
	public function setCfHeaders($cfHeaders = null)
	{
		...
		foreach ($cfHeaders as $key => $value) {
			if (amreg_substr(amreg_strtolower($key), 0, 16) === 'http_cloudfront_') {
				$this->cloudfrontHeaders[amreg_strtoupper($key)] = $value;
				$response = true;
			}
		}
	...
	private function prepareUserAgent($userAgent)
	{
		...
		$userAgent = amreg_substr($userAgent, 0, 500);
		return $userAgent;
	}
			...
			if (isset($this->httpHeaders[$mobileHeader])) {
				if (is_array($matchType['matches'])) {
					foreach ($matchType['matches'] as $_match) {
						if (amreg_strpos($this->httpHeaders[$mobileHeader], $_match) !== false) {
							return true;
						}
					}
	...
	public function __call($name, $arguments)
	{
		...
		if (amreg_substr($name, 0, 2) !== 'is') {
			throw new BadMethodCallException("No such method exists: $name");
		}

		$this->setDetectionType(self::DETECTION_TYPE_MOBILE);

		$key = amreg_substr($name, 2);
