...
class Country
{
	const USER_TYPE = 'KitMultiRegionsCountry';
	...
	public static function GetUserTypeDescription()
	{
		return array(
			...
			"DESCRIPTION" => Loc::getMessage("KIT_REGION_IBPROP_COUNTRY_DESCRIPTION"),
	...
	public static function GetPropertyFieldHtml($arProperty, $arValue, $arHTMLControlName)
	{
		...
		<div class="bammultiregionsadm-area-item">
			...
			<input type="text" name="TEXTFIELD_<?= htmlspecialcharsbx($arHTMLControlName['VALUE']) ?>" id="TEXTFIELD_<?= htmlspecialcharsbx($ident) ?>" size="50" value="<?= $arValue['VALUE_TEXT'] ?>" data-action="country" data-result-id="<?= htmlspecialcharsbx($ident) ?>" data-min-length="2" data-cnt="30" class="amr-request-field" autocomplete="off"/>
		</div>
	...
	public static function GetPropertyFieldHtmlMulty($arProperty, $arValue, $arHTMLControlName)
	{
		...
		foreach ($arValue as $intPropertyValueID => $arOneValue) {
			...
			<div class="bammultiregionsadm-area-item">
				...
				<input type="text" name="TEXTFIELD_<?= htmlspecialcharsbx($strFieldName) ?>" id="TEXTFIELD_<?= htmlspecialcharsbx($ident) ?>" size="50" value="<?= $arOneValue['VALUE_TEXT'] ?>" data-action="country" data-result-id="<?= htmlspecialcharsbx($ident) ?>" data-min-length="2" data-cnt="30" class="amr-request-field" autocomplete="off"/>
			</div>
			<div clas="bammultiregionsadm-area-item-clear"></div>
			<?
		}
		...
		if ((int)$arProperty['MULTIPLE_CNT'] > 0) {
			for ($i = 0; $i < $arProperty['MULTIPLE_CNT']; $i++) {
				...
				<div class="bammultiregionsadm-area-item">
					...
					<input type="text" name="TEXTFIELD_<?= htmlspecialcharsbx($strFieldName) ?>" id="TEXTFIELD_<?= htmlspecialcharsbx($ident) ?>" size="50" value="" data-action="country" data-result-id="<?= htmlspecialcharsbx($ident) ?>" data-min-length="2" data-cnt="30" class="amr-request-field" autocomplete="off"/>
				</div>
				<div clas="bammultiregionsadm-area-item-clear"></div>
				<?
			}
	...
	protected static function getTextValue($ID)
	{
		...
		if ($ID > 0) {
			$arData = array(
				"ID" => $ID,
				"NAME" => \CKitMultiRegions::getFirstNotEmpty(CountryLangTable::getLangNames($ID)),
			);
	...
	public static function GetAdminListViewHTML($arProperty, $arValue, $arHTMLControlName)
	{
		...
		if (is_array($arResult)) {
			$strResult = '<a href="/bitrix/admin/kit.multiregions.country.edit.php?ID=' . $arResult['ID'] . '" title="' . Loc::getMessage("MAIN_EDIT") . '">' . $arResult['NAME'] . '</a>';
		}
		return $strResult;
	}
