...
$sTableID = "tbl_kit_multiregions_content";
...
$arTypesList = array();
$rTypes = \Kit\MultiRegions\ContentTypesTable::getList(array(
...
$filterFields = array(
	...
	array(
		"id" => "SITE_ID",
		"name" => Loc::getMessage("KIT_IP_FILTER_SITE_ID"),
		"filterable" => "",
		"type" => "list",
		"items" => $arSitesList,
		"default" => true,
	...
	array(
		"id" => "TYPE_ID",
		"name" => Loc::getMessage("KIT_MULTIREGIONS_FILTER_TYPE_ID"),
		"filterable" => "",
		"type" => "list",
		"items" => $arTypesList,
		"default" => true,
	...
	array(
		"id" => "CITY.CODE",
		"name" => Loc::getMessage("KIT_MULTIREGIONS_FILTER_CITY_CODE"),
		"filterable" => "?",
		"quickSearch" => "?",
		"default" => false,
	),
	...
	array(
		"id" => "DOMAIN_ID",
		"name" => Loc::getMessage("KIT_MULTIREGIONS_FILTER_DOMAIN_ID"),
		"filterable" => "",
		"type" => "list",
		"items" => $arDomainList,
		"default" => true,
	...
	array(
		"id" => "CONTENT",
		"name" => Loc::getMessage("KIT_MULTIREGIONS_FILTER_CONTENT"),
		"filterable" => "?",
		"quickSearch" => "?",
		"default" => false,
	),
...
if ($lAdmin->EditAction()) {
	foreach ($FIELDS as $ID => $postFields) {
		...
		$oUpdate = \Kit\MultiRegions\ContentTable::update($ID, $arFields);
...
if (($arID = $lAdmin->GroupAction()) && $modulePermissions >= "W") {
	if ($_REQUEST['action_target'] == 'selected') {
		$arID = Array();
		$dbResultList = \Kit\MultiRegions\ContentTable::getList(array(
			"order" => $arOrder,
			"filter" => $arFilter,
			"select" => array("ID")));
	...
	foreach ($arID as $ID) {
		...
		switch ($_REQUEST['action']) {
			case "delete":
				@set_time_limit(0);
				$rRecord = \Kit\MultiRegions\ContentTable::getList(array(
					"filter" => array("ID" => $ID),
					"select" => array("ID"),
				));
				$arRecordOld = $rRecord->Fetch();
				$DB->StartTransaction();
				$rOperation = \Kit\MultiRegions\ContentTable::delete($ID);
			...
			case "deactivate":
				...
				$oResult = \Kit\MultiRegions\ContentTable::update($ID, $arFields);
				if (!$oResult->isSuccess()) {
					$lAdmin->AddGroupError(GetMessage("KIT_MULTIREGIONS_UPDATE_ERROR") . implode(", ", $oResult->getErrorMessages()), $ID);
				}
				break;
		}
	}
...
$arHeader = array(
	...
	array(
		"id" => "TYPE_ID",
		"content" => Loc::getMessage("KIT_MULTIREGIONS_FIELD_TYPE_ID"),
		"sort" => "TYPE_ID",
		"default" => true,
	),
	array(
	...
	array(
		"id" => "DOMAIN_ID",
		"content" => Loc::getMessage("KIT_MULTIREGIONS_FIELD_DOMAIN_ID"),
		"sort" => "DOMAIN_ID",
		"default" => true,
	),
	array(
	...
	array(
		"id" => "CONTENT",
		"content" => Loc::getMessage("KIT_MULTIREGIONS_FIELD_CONTENT"),
		"sort" => "CONTENT",
		"default" => true,
	),
);
...
$rsItems = \Kit\MultiRegions\ContentTable::getList(array(
	"order" => $arOrder,
	"filter" => $arFilter,
...
while ($arData = $rsItems->NavNext()) {
	...
	$row =& $lAdmin->AddRow($arData['ID'], $arData, 'kit.multiregions.content.edit.php?ID=' . $arData['ID'] . '&lang=' . LANGUAGE_ID, Loc::getMessage("KIT_MULTIREGIONS_RECORD_EDIT"));
	...
	$row->AddViewField("ID", '<a href="kit.multiregions.content.edit.php?ID=' . $arData['ID'] . '&lang=' . LANGUAGE_ID . '">' . $arData['ID'] . '</a>');
	$row->AddViewField("TYPE_ID", $arData['TYPE_ID'] > 0 ? '[' . $arData['TYPE_ID'] . '] <a href="/bitrix/admin/kit.multiregions.content.types.edit.php?ID=' . $arData['TYPE_ID'] . '&lang=' . LANGUAGE_ID . '">' . $arData['TYPE_NAME'] . '</a>' : "");
	...
	$row->AddViewField("DOMAIN_ID", $arData['DOMAIN_ID'] > 0 ? '[' . $arData['DOMAIN_ID'] . '] <a href="/bitrix/admin/kit.multiregions.domain.edit.php?ID=' . $arData['DOMAIN_ID'] . '&lang=' . LANGUAGE_ID . '">' . $arData['DOMAIN_NAME'] . ' (' . $arData['DOMAIN_DOMAIN'] . ')</a>' : "");
	...
	if ($modulePermissions >= "W") {
		$arActions[] = array(
			...
			"DEFAULT" => true,
			"ACTION" => $lAdmin->ActionRedirect("kit.multiregions.content.edit.php?ID=" . $arData['ID'] . "&lang=" . LANGUAGE_ID),
		);
	}
...
if ($modulePermissions >= "W") {
	$aContext = array(
		array(
			...
			"ICON" => "btn_new",
			"LINK" => "kit.multiregions.content.edit.php?lang=" . LANGUAGE_ID,
