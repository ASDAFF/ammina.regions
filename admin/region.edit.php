...
$arCurrentItem = false;
if ($ID > 0) {
	$arCurrentItem = \Ammina\Regions\RegionTable::getById($ID)->fetch();
}
if (!$arCurrentItem) {
	$isNewItem = true;
	$ID = false;
...
if ($isNewItem) {
	LocalRedirect("/bitrix/admin/ammina.regions.region.php?lang=" . LANGUAGE_ID);
}
$result = new \Bitrix\Main\Entity\Result();

$customTabber = new CAdminTabEngine("OnAdminAmminaRegionsRegionEdit");
$customDraggableBlocks = new CAdminDraggableBlockEngine('OnAdminAmminaRegionsRegionEditDraggable');
...
if ($isSavingOperation) {
	...
	if ($isNewItem) {
		$oTableResult = \Ammina\Regions\RegionTable::add($_POST['FIELDS']);
		$ID = $oTableResult->getId();
	} else {
		$oTableResult = \Ammina\Regions\RegionTable::update($ID, $_POST['FIELDS']);
	}
	...
	if ($result->isSuccess()) {
		if (isset($_POST["save"])) {
			LocalRedirect("/bitrix/admin/ammina.regions.region.php?lang=" . LANGUAGE_ID . GetFilterParams("filter_", false));
		} else {
			LocalRedirect("/bitrix/admin/ammina.regions.region.edit.php?lang=" . LANGUAGE_ID . "&ID=" . $ID . GetFilterParams("filter_", false));
		}
	}
}
...
$aMenu[] = array(
	...
	"LINK" => "/bitrix/admin/ammina.regions.region.php?lang=" . LANGUAGE_ID . GetFilterParams("filter_"),
);
...
$formId = "ammina_regions_region_edit";

$aTabs = array(
	array("DIV" => "tab_ammina", "TAB" => Loc::getMessage("AMMINA_REGIONS_TAB_REGION"), "SHOW_WRAP" => "N", "IS_DRAGGABLE" => "Y"),
);
		...
		<td>
			...
			<div style="position: relative; vertical-align: top">
				...
				foreach ($blocksPage as $blockCode) {
					...
					switch ($blockCode) {
						case "region":
							echo \Ammina\Regions\Helpers\Admin\Blocks\Region::getEdit($arCurrentItem);
							break;
						default:
...
$tabControl->Buttons(
	array(
		"back_url" => "/bitrix/admin/ammina.regions.region.php?lang=" . LANGUAGE_ID . GetFilterParams("filter_"))
);
