...
$arCurrentItem = false;
if ($ID > 0) {
	$arCurrentItem = \Kit\MultiRegions\ContentTable::getById($ID)->fetch();
}
if (!$arCurrentItem) {
	$isNewItem = true;
	$ID = false;
...
$customTabber = new CAdminTabEngine("OnAdminKitMultiRegionsContentEdit");
$customDraggableBlocks = new CAdminDraggableBlockEngine('OnAdminKitMultiRegionsContentEditDraggable');
...
if ($isSavingOperation) {
	...
	if ($isNewItem) {
		$oTableResult = \Kit\MultiRegions\ContentTable::add($arFields);
		$ID = $oTableResult->getId();
	} else {
		$oTableResult = \Kit\MultiRegions\ContentTable::update($ID, $arFields);
	}
	...
	if ($result->isSuccess()) {
		if (isset($_POST["save"])) {
			LocalRedirect("/bitrix/admin/kit.multiregions.content.php?lang=" . LANGUAGE_ID . GetFilterParams("filter_", false));
		} else {
			LocalRedirect("/bitrix/admin/kit.multiregions.content.edit.php?lang=" . LANGUAGE_ID . "&ID=" . $ID . GetFilterParams("filter_", false));
		}
	}
}
...
$aMenu[] = array(
	...
	"LINK" => "/bitrix/admin/kit.multiregions.content.php?lang=" . LANGUAGE_ID . GetFilterParams("filter_"),
);
...
$formId = "kit_multiregions_content_edit";

$aTabs = array(
	array("DIV" => "tab_kit", "TAB" => Loc::getMessage("KIT_MULTIREGIONS_TAB_CONTENT"), "SHOW_WRAP" => "N", "IS_DRAGGABLE" => "Y"),
);
echo \Kit\MultiRegions\Helpers\Admin\Blocks\Content::getScripts();
echo \Kit\MultiRegions\Helpers\Admin\Blocks\ContentExt::getScripts();
?>
		...
		<td>
			...
			<div style="position: relative; vertical-align: top">
				...
				foreach ($blocksPage as $blockCode) {
					...
					switch ($blockCode) {
						case "content":
							echo \Kit\MultiRegions\Helpers\Admin\Blocks\Content::getEdit($arCurrentItem);
							break;
						case "content_ext":
							echo \Kit\MultiRegions\Helpers\Admin\Blocks\ContentExt::getEdit($arCurrentItem);
							break;
						default:
...
$tabControl->Buttons(
	array(
		"back_url" => "/bitrix/admin/kit.multiregions.content.php?lang=" . LANGUAGE_ID . GetFilterParams("filter_"))
);
